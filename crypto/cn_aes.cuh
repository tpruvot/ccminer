...
static const __device__ __align__(16) uint32_t TestTable1[256] = {
...
static const __device__ __align__(16) uint32_t TestTable2[256] = {
...
static const __device__ __align__(16) uint32_t TestTable3[256] = {
...
static const __device__ __align__(16) uint32_t TestTable4[256] = {
...
#if 1
__device__ __forceinline__
void cn_aes_single_round_c(const uint32_t * __restrict__ st1, const uint4 key, uint4 * __restrict__ res, const uchar4 * __restrict__ in)
{
/* One 256 table */
	res->x = key.x ^ st1[in[0].x] ^ (st1[in[1].y] << 8 | st1[in[1].y] >> 24) ^ (st1[in[2].z] << 16 | st1[in[2].z] >> 16) ^ (st1[in[3].w] << 24 | st1[in[3].w] >> 8);
	res->y = key.y ^ (st1[in[0].w] << 24 | st1[in[0].w] >> 8) ^ st1[in[1].x] ^ (st1[in[2].y] << 8 | st1[in[2].y] >> 24) ^ (st1[in[3].z] << 16 | st1[in[3].z] >> 16);
	res->z = key.z ^ (st1[in[0].z] << 16 | st1[in[0].z] >> 16) ^ (st1[in[1].w] << 24 | st1[in[1].w] >> 8) ^ st1[in[2].x] ^ (st1[in[3].y] << 8 | st1[in[3].y] >> 24);
	res->w = key.w ^ (st1[in[0].y] << 8 | st1[in[0].y] >> 24) ^ (st1[in[1].z] << 16 | st1[in[1].z] >> 16) ^ (st1[in[2].w] << 24 | st1[in[2].w] >> 8) ^ st1[in[3].x];
}
#else
__device__ __forceinline__
void cn_aes_single_round_c(const uint32_t * __restrict__ st1, const uint32_t * __restrict__ st2,
	const uint32_t * __restrict__ st3, const uint32_t * __restrict__ st4, const uint4 key,
	uint4 * __restrict__ res, const uchar4 * __restrict__ in)
{
/* Four 256 table */
	res->x = key.x ^ st1[in[0].x] ^ st2[in[1].y] ^ st3[in[2].z] ^ st4[in[3].w];
	res->y = key.y ^ st4[in[0].w] ^ st1[in[1].x] ^ st2[in[2].y] ^ st3[in[3].z];
	res->z = key.z ^ st3[in[0].z] ^ st4[in[1].w] ^ st1[in[2].x] ^ st2[in[3].y];
	res->w = key.w ^ st2[in[0].y] ^ st3[in[1].z] ^ st4[in[2].w] ^ st1[in[3].x];
}
#endif
...
*/
/* Four 256 table init
__device__ __forceinline__ void cn_aes_gpu_init4(uint32_t *s1, uint32_t *s2, uint32_t *s3, uint32_t *s4)
{
...
*/
/* One 256 table init */
__device__ __forceinline__ void cn_aes_gpu_init4(uint32_t *s1)
{
	for (int i = threadIdx.x; i < 256; i += blockDim.x)
		s1[i] = TestTable1[i];
}

#if 1 /* One 1024 table init */
